...
const serviciosNav: NavItem[] = [
  { slug: "geologia", titulo: "Estudios de Geología", icon: <Mountain className="w-4 h-4" /> },

  { slug: "geotecnia", titulo: "Estudios de Geotecnia", icon: <Hammer className="w-4 h-4" /> },
  ...
  { slug: "geofisica", titulo: "Estudios de Geofísica", icon: <ActivitySquare className="w-4 h-4" /> },

  { slug: "hidrogeologia", titulo: "Hidrología e Hidrogeología", icon: <Waves className="w-4 h-4" /> },

  { slug: "geomecanica", titulo: "Estudios de Geomecánica", icon: <MountainSnow className="w-4 h-4" /> },

  { slug: "ensayo-de-campo", titulo: "Ensayos de Campo", icon: <MapPin className="w-4 h-4" /> },

  { slug: "mecanica-de-suelos", titulo: "Mecánica de Suelos", icon: <Layers className="w-4 h-4" /> },

  { slug: "control-de-calidad", titulo: "Control de Calidad en Obras", icon: <ShieldCheck className="w-4 h-4" /> },

  { slug: "evaluacion-estructural", titulo: "Evaluación Estructural", icon: <Building2 className="w-4 h-4" /> },
];
...
const ServicioNav = () => {
  ...
  const getHref = (item: NavItem) => item.link || `/servicios/${item.slug}`;

  const isActive = (item: NavItem) => {
    if (item.link) return pathname === item.link;
    return pathname === `/servicios/${item.slug}` || pathname?.startsWith(`/servicios/${item.slug}/`);
  };
  ...
  return (
    <div className="w-full bg-gradient-to-r from-gray-50 to-blue-50 border-b border-gray-200 -mt-1 relative z-10 py-7">
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="relative md:hidden">
          ...
          <div className="pointer-events-none absolute inset-y-0 right-0 w-8 bg-gradient-to-l from-blue-50 to-transparent" />
              ...
              return (
                ...
                <Link
                  ...
                  className={`
                    ...
                    ${active ? "text-white border-transparent" : "text-[#1b4772] border-gray-200"}
                  `}
                  style={{
                    background: active
                ...
                >
                  ...
                  <div className="relative z-[1] flex items-center gap-3">
                    ...
                    <div className={`flex items-center justify-center w-9 h-9 rounded-full ${active ? "bg-white/20" : "bg-[#1b4772]/10"}`}>
                      <span className={active ? "text-white" : "text-[#1b4772]"}>{s.icon}</span>
                    </div>

                    <span
                      className={`text-[13px] font-semibold leading-snug ${active ? "text-white" : "text-[#1b4772]"}`}
                      style={{
                        display: "-webkit-box",
                        WebkitLineClamp: 2,
                  ...
                  <div className={`absolute -bottom-2 left-4 right-4 h-1 rounded-full ${active ? "bg-white/70" : "bg-transparent"}`} />
        ...
        <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-5 md:gap-4 [--cas-azul:#1b4772] [--cas-azul-2:#2c5282]">
            ...
            return (
              ...
              <Link
                ...
                className={`
                  ...
                  focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1b4772] focus-visible:ring-offset-2
                  ${active ? "text-white border-transparent" : "text-[#1b4772] border-gray-200 hover:border-[#1b4772]"}
                `}
                style={{
                  background: active
                    ? "linear-gradient(135deg, var(--cas-azul) 0%, var(--cas-azul-2) 100%)"
